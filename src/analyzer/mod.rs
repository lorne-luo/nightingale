...
impl AnalysisQueue {
    pub fn enqueue(&mut self, song_index: usize) {
        if self
            .active
            .as_ref()
            .is_some_and(|a| a.song_index == song_index)
        {
            return;
        }
...
fn spawn_server() -> Result<ServerProcess, NightingaleError> {
    ...
    let stdin = BufWriter::new(child.stdin.take().ok_or(NightingaleError::Other(
        "Failed to capture server stdin".into(),
    ))?);
    let stdout = BufReader::new(child.stdout.take().ok_or(NightingaleError::Other(
...
fn process_queue(
    ...
    let lang_override = config
        .language_override(&song.file_hash)
        .map(|s| s.to_string());
...
fn poll_active_job(
    ...
    if success && cache.transcript_exists(&library.songs[song_index].file_hash) {
        info!(
            ...
            library.songs[song_index].path.display()
        );
        ...
        library.songs[song_index].analysis_status = AnalysisStatus::Failed(finished_info.message);
    }
}

fn kill_analyzer_on_exit(mut exit_events: MessageReader<AppExit>, queue: Res<AnalysisQueue>) {
    ...
    match ANALYZER_SERVER.try_lock() {
        ...
        Err(_) => {
            eprintln!(
                "[analyzer] Server lock held by analysis thread, process already killed by signal"
            );
        }
    }
}
